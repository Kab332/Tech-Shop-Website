    ...
    if (itemFlag.text() === "true") {
        $.ajax({
            ...
            success: function (response) {
                ...
                //draws the first row
                table.append('<tr><th>name</th><th>quantity</th><th>date</th><th>type</th><th>remove?</th><th><form method="post" action="/removeAllItems" class="form"><button class="control row 3">Remove All</button></th>');
                ...
                for (var i = 0; i < response.length; i++) {
                    ...
                    var tr = $("<tr>").append('<td>' + response[i].name + '</td><td>' + response[i].quantity + '</td><td>' + response[i].date + '</td><td>' + response[i].type + '</td>');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                for (var i = 0; i < response.length; i++) {
                    var remove = $('<form method="post" action="/removeItem" class="form">');
                    remove.append('<input type="hidden" name="name" value="' + response[i].username + '"><button>Remove</button>');
                    var tr = $("<tr>").append('<td>' + response[i].username + '</td><td>' + response[i].email + '</td><td>' + response[i].hashedPassword + '</td><td>' + response[i].address + '</td><td>' + response[i].country + '</td><td>' + response[i].province + '</td><td>' + response[i].city + '</td><td>' + response[i].zip + '</td>');
                    var td = $("<td>").append(remove)
                    tr.append(td);
                    table.append(tr)
                }
